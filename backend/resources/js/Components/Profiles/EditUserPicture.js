...
import { setUserProfileInfo, setUserProfileFieldsEmptyErrors, setUserProfilePicInputs } from '../../Store/Actions/userProfileActions';
...
class EditUserPicture extends Component {
    ...
    async logoUploadHandler(file) {
        ...
        try {
            ...
            this.props.setUserProfilePicInputs(logoUploadResponse.data.data.profile_pic);
            ...
            await this.setState({
                ...this.state,
                hasFieldError: true,
                errorMessage: error.response.data?.profile_pic ? error.response.data.profile_pic.join(",") : error.response.statusText,
                imageUpload: false
            });
    ...
    render() {
        ...
        const userPic = this.props.profilePicInputs === null 
                        || this.props.profilePicInputs === '' 
                        ? 
                        '' 
                        : 
                        (
                            this.props.profilePicInputs.includes(process.env.MIX_TOOLBOX_MEDIA_URL,0) 
                            ? 
                            this.props.profilePicInputs
                            : 
                            process.env.MIX_TOOLBOX_MEDIA_URL+'/'+this.props.profilePicInputs
                        );
        ...
        const errorMessage = this.state.hasFieldError === true ?
            <p className="at2_error_text">{this.state.errorMessage == 'The profile pic failed to upload.' ? this.state.errorMessage + '(types must be :jpeg,png,jpg,gif,svg and max size: 2048 kb )' : ''}</p> : '';
