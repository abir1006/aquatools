...
class PriceModulePopup extends Component {
    ...
    componentDidMount() {
        if (this.props.priceModuleInputs !== undefined) {
            ...
            caseNumbers.map(caseNo => {
                ...
                if (!Boolean(priceModuleInputs['price_module_cv_case' + caseNo])) {
                    priceModuleInputs['price_module_cv_case' + caseNo] = Boolean(defaultCV['Case'+caseNo]) ? defaultCV['Case'+caseNo] : 22;
                }
                if (!Boolean(priceModuleInputs['price_module_snittvekt_case' + caseNo])) {
                    priceModuleInputs['price_module_snittvekt_case' + caseNo] = Boolean(defaultSnittvekt['Case'+caseNo]) ? defaultSnittvekt['Case'+caseNo] : 4.74;
                }
            });
    ...
    historicPeriodStart(name, id) {
        ...
        this.props.setPriceModuleInputs({
            historic_period_end: (parseInt(selectedYear) + 1) + '-' + selectedWeek
        });

        priceModuleInputs.historic_period_end = (parseInt(selectedYear) + 1) + '-' + selectedWeek;
    ...
    async addPriceHandler(e) {
        ...
        await this.props.addMTBLaksePrice(this.props.outputs, this.props.caseNumbers, NavService.getCurrentRoute());
    ...
    render() {
        ...
        years1.map((y, kn1) => {
            let totalWeeks1 = kn1 === (years1.length - 1) ? currentWeek : 52;
            for (let w = 1; w <= 52; w++) {
                w = w < 10 ? '0' + w : w;
        ...
        years2.map((y, kn2) => {
            let totalWeeks2 = kn2 === (years2.length - 1) ? currentWeek : 52;
            for (let w = 1; w <= totalWeeks2; w++) {
                w = w < 10 ? '0' + w : w;
        ...
        forwardYears2.map((y, keyNumber2) => {
            let cm2 = keyNumber2 === 0 ? currentMonth : 1;
            for (let m = cm2; m <= 12; m++) {
                m = m < 10 ? '0' + m : m;
                yearMonths2.push({id: y.name + '-' + m, name: y.name + '-' + m});
            }
        })

        const inputPriceType = this.props.priceModuleInputs === undefined || this.props.priceModuleInputs.price_type === undefined ? 'Historic' : this.props.priceModuleInputs.price_type;

        const inputForwardPeriodStart = this.props.priceModuleInputs === undefined || this.props.priceModuleInputs.forward_period_start === undefined ? forwardSelectedStart : this.props.priceModuleInputs.forward_period_start;
        const inputForwardPeriodEnd = this.props.priceModuleInputs === undefined || this.props.priceModuleInputs.forward_period_end === undefined ? forwardSelectedEnd : this.props.priceModuleInputs.forward_period_end;

        const inputHistoricPeriodStart = this.props.priceModuleInputs === undefined || this.props.priceModuleInputs.historic_period_start === undefined ? historicSelectedStart : this.props.priceModuleInputs.historic_period_start;
        const inputHistoricPeriodEnd = this.props.priceModuleInputs === undefined || this.props.priceModuleInputs.historic_period_end === undefined ? historicSelectedEnd : this.props.priceModuleInputs.historic_period_end;
                        ...
                        <div className="form-row">
                            <div className="col-12 col-xl-12 col-lg-12 text-left">
                                <label className="col-form-label mt-2 pb-2" htmlFor="">
                                    ...
                                    <b>{t('forward_salmon_price')}: {forwardSalmonPrice} <br /> {t('adjusted_forward_salmon_price')}: {adjustForwardSalmonPrice}</b>}
                                </label>
                            </div>
                        </div>
