...
class MainNavigation extends Component {
    ...
    render() {
        ...
        let firstName = this.props.data.length === 0 || this.props.data.user.first_name === null ? '' : this.props.data.user.first_name;
        let lastName = this.props.data.length === 0 || this.props.data.user.last_name === undefined || this.props.data.user.last_name === null ? '' : this.props.data.user.last_name;
        let fullName = firstName + ' ' + lastName;
        let firstNameFirstLetter = firstName && firstName.substr(0, 1);
        ...
        const profilePic = this.props.data.user === undefined || this.props.data.user.profile_pic === undefined ? '' : this.props.data.user.profile_pic;
                        ...
                        <div tabIndex="1" id="user-logged-panel" onClick={e => this.props.handleProfileClick(e)}
                            ...
                            <div className="user-img-panel">
                                <div className="user-name-img">
                                    <UserProfilePic img={profilePic} sName={shortName} />
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <ul className="nav d-flex align-items-center justify-content-between">
                            {
                                ...
                                <li className={this.state.currentRouteName === 'settings' ? 'nav-item current-nav-menu  flex-grow-1' : 'nav-item  flex-grow-1'}>
                                    <Link style={{ padding: '6px 0 6px 33px' }} className="nav-link menu-settings-icon pr-1" to="/admin/settings">{t('settings')}</Link>
                                </li>
                            }
...
function UserProfilePic(props) {
    const userPicSrc = props.img === null || props.img === '' ? null : props.img;
    if (userPicSrc === null) {
        return props.sName;

    } else {
        
        let url = userPicSrc.includes(process.env.MIX_TOOLBOX_MEDIA_URL,0) ? userPicSrc: process.env.MIX_TOOLBOX_MEDIA_URL+'/'+userPicSrc
        
        return <img src={url} className="user-name-img" />
    }
