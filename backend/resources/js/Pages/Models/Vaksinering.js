...
import { setVaccineNames } from "../../Store/Actions/VaccineModelActions";
import {
...
import { hideInfoPopup, hidePDFPopup, hidePPTPopup, hidePriceModulePopup, hideConfirmPopup } from "../../Store/Actions/popupActions";
import ModelActionTabs from "../../Components/Model/Vaksinering/ModelActionTabs/ModelActionTabs";
import BlocksInputs from "../../Components/Model/Vaksinering/BlocksInputs/BlocksInputs";
...
import ModelOutput from "../../Components/Model/Vaksinering/ModelOutput/ModelOutput";
...
class VaksineringModel extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    checkDirty(oldValus, newValus) {
        ...
        const isUpdated = (oldValues, newValues) => Object.keys(oldValues).filter(key => {
            ...
            if (key === 'vaksinering_general_navn_case1' || key === 'vaksinering_general_navn_case2' || key === 'vaksinering_general_navn_case3')
                return false;
    ...
    async checkAuthAndRouteAccess() {
        ...
        //check if user has Vaccine model access
        ...
        if (currentRoute === 'vaksinering') {
            await this.props.checkRouteAccess(this.props.history, hasModelPermission, 'Vaccination');
        }

        if (hasModelPermission) {
            await this.props.setPriceModuleCurrentModel(currentRoute);
            await this.props.setModelScreenCases([1, 2, 3]);
            await this.props.setVaccineNames([{ name: 'A' }]);
    ...
    async pdfDownloadHandler(sendToEmail = false, sendToMe = false) {
        ...
        try {
            ...
            const inputArray = this.props.inputs;
            inputArray.vaksinering_produksjonsmodell_antall_smolt_case1 = inputArray.vaksinering_produksjonsmodell_antall_smolt_case1.toString().split(' ').join('');

            const pdfResponse = await axios({
                url: 'api/vaccine/download-pdf',
                method: 'POST',
                data: {
            ...
            if (sendToEmail === false && sendToMe === false) {
                ...
                link.href = pdfUrl;
                link.setAttribute('download', 'Vaccine_' + this.props.auth.data.user.id + '_' + this.props.auth.data.user.company.id + '_' + DateTimeService.getCurrentDateTimeForPDF() + '.pdf');
    ...
    async pptDownloadHandler(sendToEmail = false, sendToMe = false) {
        ...
        try {
            ...
            if (sendToEmail === false && sendToMe === false) {
                ...
                const link = document.createElement('a');
                link.href = pptUrl;
                link.setAttribute('download', 'Vaccine_' + this.props.auth.data.user.id + '_' + this.props.auth.data.user.company.id + '_' + DateTimeService.getCurrentDateTimeForPDF() + '.pptx');
                document.body.appendChild(link);
                link.click();
                this.props.hidePPTPopup();
            }
...
export default connect(mapStateToProps, {
    ...
    setIsDirty,
    unsetIsDirty
})(withTranslation()(VaksineringModel));
