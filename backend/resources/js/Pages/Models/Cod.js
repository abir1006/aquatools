...
import ModelActionTabs from "../../Components/Model/Cod/ModelActionTabs/ModelActionTabs";
import BlocksInputs from "../../Components/Model/Cod/BlocksInputs/BlocksInputs";
...
import ModelOutput from "../../Components/Model/Cod/ModelOutput/ModelOutput";
...
class CodModel extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    async checkAuthAndRouteAccess() {
        ...
        if (currentRoute === 'cost_of_disease') {
            await this.props.checkRouteAccess(this.props.history, hasModelPermission, 'Cost of disease');
        }
    ...
    async pdfDownloadHandler(sendToEmail = false, sendToMe = false) {
        ...
        try {
            ...
            inputArray.cost_of_disease_produksjonsmodell_antall_smolt_case1 = inputArray.cost_of_disease_produksjonsmodell_antall_smolt_case1.toString().split(' ').join('');


            const pdfResponse = await axios({
                url: 'api/cod/download-pdf',
                method: 'POST',
                data: {
            ...
            if (sendToEmail === false && sendToMe === false) {
                ...
                link.href = pdfUrl;
                link.setAttribute('download', 'Cod_' + this.props.auth.data.user.id + '_' + this.props.auth.data.user.company.id + '_' + DateTimeService.getCurrentDateTimeForPDF() + '.pdf');
    ...
    async pptDownloadHandler(sendToEmail = false, sendToMe = false) {
        ...
        try {
            ...
            if (sendToEmail === false && sendToMe === false) {
                ...
                const link = document.createElement('a');
                link.href = pptUrl;
                link.setAttribute('download', 'Cod_' + this.props.auth.data.user.id + '_' + this.props.auth.data.user.company.id + '_' + DateTimeService.getCurrentDateTimeForPDF() + '.pptx');
                document.body.appendChild(link);
                link.click();
                this.props.hidePPTPopup();
            }
