...
import ModelActionTabs from "../../Components/Model/Optimalisering/ModelActionTabs/ModelActionTabs";
import BlocksInputs from "../../Components/Model/Optimalisering/BlocksInputs/BlocksInputs";
...
import ModelOutput from "../../Components/Model/Optimalisering/ModelOutput/ModelOutput";
...
class OptimaliseringModel extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    async checkAuthAndRouteAccess() {
        ...
        if (currentRoute === 'optimalisering') {
            await this.props.checkRouteAccess(this.props.history, hasModelPermission, 'Optimization');
        }
    ...
    async pdfDownloadHandler(sendToEmail = false, sendToMe = false) {
        ...
        try {
            ...
            inputArray.optimalisering_produksjonsmodell_antall_smolt_case1 = inputArray.optimalisering_produksjonsmodell_antall_smolt_case1.toString().split(' ').join('');

            const pdfResponse = await axios({
                url: 'api/optimalisering/download-pdf',
                method: 'POST',
                data: {
            ...
            if (sendToEmail === false && sendToMe === false) {
                ...
                link.href = pdfUrl;
                link.setAttribute('download', 'Optimization_' + this.props.auth.data.user.id + '_' + this.props.auth.data.user.company.id + '_' + DateTimeService.getCurrentDateTimeForPDF() + '.pdf');
    ...
    async pptDownloadHandler(sendToEmail = false, sendToMe = false) {
        ...
        try {
            ...
            const pptResponse = await axios({
                url: 'api/optimalisering/download-ppt',
                method: 'POST',
                data: {
            ...
            if (sendToEmail === false && sendToMe === false) {
                ...
                const link = document.createElement('a');
                link.href = pptUrl;
                link.setAttribute('download', 'Optimization_' + this.props.auth.data.user.id + '_' + this.props.auth.data.user.company.id + '_' + DateTimeService.getCurrentDateTimeForPDF() + '.pptx');
                document.body.appendChild(link);
                link.click();
                this.props.hidePPTPopup();
            }
...
export default connect(mapStateToProps, {
    ...
    setIsDirty,
    unsetIsDirty
})(withTranslation()(OptimaliseringModel));
