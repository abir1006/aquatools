...
let cancelToken;
export const setOptModelResult = (allInputObject, allCases, pageName) => async dispatch => {
    ...
    try {
        ...
        data.optimalisering_produksjonsmodell_antall_smolt_case1 = data.optimalisering_produksjonsmodell_antall_smolt_case1.toString().split(' ').join('');
        data.total_cases = allCases.length;

        //Save the cancel token for the current request
        cancelToken = axios.CancelToken.source();

        let apiUrl = 'api/optimalisering/calculation';
        if (Boolean(pageName)) {
            apiUrl = 'api/optimalisering/calculation?pageName=' + pageName;
        }
