...
const BarCustomizedLabel = props => {
    ...
    if (resultName === 'okteUtgifter' && Math.round(viewBox.height) == 0) {
        yAxisValue = yAxisValue - 2;
    }

    if (resultName === 'biologiskeTap' && Math.round(viewBox.height) == 0) {
        yAxisValue = yAxisValue - 2;
    }

    if (resultName === 'okteUtgifter' && Math.round(viewBox.height) > 0 && Math.round(viewBox.height) <= 5) {
        yAxisValue = yAxisValue - 10;
    }

    if (resultName === 'biologiskeTap' && Math.round(viewBox.height) > 0 && Math.round(viewBox.height) <= 5) {
        yAxisValue = yAxisValue - 2;
    }
