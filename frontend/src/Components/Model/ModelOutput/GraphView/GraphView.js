...
class GraphView extends Component {
    ...
    render() {
        ...
        let graphBaseValue = this.props.graphBaseValue;
        ...
        for (let index in graphOutput) {
            ...
            for (let caseStr in graphOutput[index]) {
                ...
                if (graphOutput[index][caseStr] < graphBaseValue[index]) {
                    graphBaseValue[index] = null;
                }

                tmp[countCase] = {
                    label: Boolean(this.props.modelCaseText) && Boolean(this.props.modelCaseText['case' + (countCase + 1)]) ? this.props.modelCaseText['case' + (countCase + 1)] : t(caseStr.toLocaleLowerCase()),
        ...
        const CustomTooltip = ({active, payload, label, resultName}) => {
            ...
            let labelValue = payload[index]?.value || 0.0
            ...
            let formattedValue = number_format(labelValue, dec, '.', '');

            if (labelValue.toString().split('.')[0].length >= 4) {
                formattedValue = parseInt(Math.round(labelValue))
            }

            if (labelValue.toString().split('.')[0].length === 3 && labelValue.toString().split('.')[1].length >= 1) {
                formattedValue = number_format(labelValue, 1, '.', '')
            }

            if (active) {
                return (
                    <div className="custom-tooltip">
                        <p className="label">{`${label} : ${formattedValue}${percentageStr}`}</p>
                    </div>
                );
            }
            return null;
        ...
        let newNKR = [{BCR: 0, label: 'Case1', label2: "BCR"}];
        let nkrCount = 0;
        ...
        this.props.caseNumbers.slice(1, this.props.caseNumbers.length).map(caseNo => {
            newNKR[nkrCount] = {
                ...
                label: Boolean(this.props.modelCaseText) && Boolean(this.props.modelCaseText['case' + caseNo]) ? this.props.modelCaseText['case' + caseNo] : t('case' + caseNo),
                label2: "BCR"
            }
        ...
        return (
            <div id="graph-wrapper" style={{height: graphWrapperHeight}}>
                        ...
                        return (
                            <div key={countGraph} className={classArr['col' + countColumn + 'Class']}
                                ...
                                <div className="content-block">
                                    <div className="graph-content" id={this.props.graphOutputLabel[index]}>
                                        <h5 className="output-heading">{t(this.props.graphOutputLabel[index])}
                                            {
                                                Boolean(this.props.mtbGraphHelpText[index]) && <button
                                                    type="button"
                                                    id={'help-text-id-1'}
                                                    className="btn btn-primary-outline at2-btn-no-bg at2-info-icon-btn"
                                                    onClick={e => this.viewHelpTextHandler(e, 'bcr_graph', this.props.graphOutputLabel[index])}>
                                                    i
                                                </button>
                                            }
                                        </h5>
                                        <div className="graph-plot">
                                            <ResponsiveContainer width="100%" height={graphColHeight}>
                                                <BarChart
                                                    ...
                                                    <XAxis dataKey="label"/>
                                                    {graphBaseValue[index] === null && <YAxis
                                                        type="number"
                                                        allowDataOverFlow={true}/>}
                                                    {graphBaseValue[index] !== null && <YAxis
                                                        type="number"
                                                        domain={[graphBaseValue[index], dataMax => index === 'sgr' || index === 'eFcr' || index === 'smoltVektKg' ? (dataMax + (dataMax / 7)).toFixed(2) : (dataMax + (dataMax / 7)).toFixed(0)]}
                                                        allowDataOverFlow={true}/>}
                                                    <Tooltip
                ...
                {/*                {*/}
                {/*                    Boolean(this.props.mtbGraphHelpText['bcr']) && <button*/}
                {/*                        type="button"*/}
                {/*                        id={'help-text-id-1'}*/}
                {/*                        className="btn btn-primary-outline at2-btn-no-bg at2-info-icon-btn"*/}
...
const mapStateToProps = state => ({
    ...
    mtbGraphHelpText: state.modelScreen.graphHelpText,
